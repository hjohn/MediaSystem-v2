...
public class ModelMediaGridViewCellFactory<T> implements Callback<ListView<T>, ListCell<T>> {
  private static final List<String> MEDIA_STATE_STYLES = List.of("watched", "available", "unavailable");

  public static class Model {
    public final StringProperty title = new SimpleStringProperty();
    public final StringProperty detail = new SimpleStringProperty();
  ...
  public ModelMediaGridViewCellFactory(BiConsumer<T, Model> updater) {
  ...
  public ListCell<T> call(ListView<T> param) {
    return new ListCell<>() {
...

      {
        ...
        name.textProperty().bind(model.title);
        detail.textProperty().bind(model.detail);
        asyncImageProperty.imageHandleProperty().bind(model.imageHandle);
        sideBarTopLeftText.bind(model.annotation1);
        sideBarCenterText.bind(model.annotation2);
        mediaStatusProperty.bind(model.status);
      }
      ...
      private void updateMediaStateStyles() {
        ...
        else if(mediaStatusProperty.get() == MediaStatus.AVAILABLE) {
